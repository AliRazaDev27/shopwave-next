    ...
    <Card className="flex flex-col justify-between  shadow-2xl hover:shadow-neutral-900 shadow-neutral-600 dark:shadow-neutral-600 dark:hover:shadow-neutral-400 rounded-2xl border-2 border-neutral-500">
      ...
      <CardContent>
        ...
        <div className="flex gap-4 py-2">
          ...
          {product?.rating &&
            <Badge className="text-sm text-black dark:text-white bg-orange-500">{product?.rating}</Badge>
          }
        </div>
        <div className="flex flex-wrap gap-4 py-2">
          <Badge className="text-md text-black dark:text-white bg-green-700">{product?.category?.name}</Badge>
          {product.brand &&
            <Badge className="text-md text-black dark:text-white bg-blue-700">{product?.brand?.name}</Badge>
          }
        </div>
        <div className="flex gap-4 py-2">
          <Badge className="font-semibold text-black dark:text-white text-lg bg-orange-600">${product?.price}</Badge>
        </div>
        <CardDescription className="pt-2 line-clamp-3">{product?.description}</CardDescription>
      </CardContent>
      <CardFooter className="flex justify-center">
        <Link href={`/productDetails/${product._id}`} prefetch={false} className="bg-orange-600 text-black
         dark:text-white px-8 py-2 rounded-md hover:bg-orange-700">Add to Cart</Link>
      </CardFooter>
    </Card>
  )
}
