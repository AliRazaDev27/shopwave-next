...
export async function usersOverview() {
  try {
    ...
    // TODO: check correct logic execution
    const active = 0
    const inactive = 0
    ...
    for (let i = 1; i <= 6; i++) {
      const count = users.filter(
        (user) => {
          ...
          const isSmaller = new Date(new Date().setMonth(currentDate.getMonth() - i))
          const isBigger = new Date(new Date().setMonth(currentDate.getMonth() - (i - 1)))
